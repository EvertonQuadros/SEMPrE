...
public class LevelScreen extends BaseScreen{
    ...
    public void initActors() {
        ...
        new Wall(-30, 0, 30, 600, mainStage); //left wall;
        new Wall(800, 0, 30, 600, mainStage); //right wall;
        new Wall(0, 600, 800, 50, mainStage); //top wall;
        
        Brick tempBrick = new Brick(0, 0, mainStage, Color.WHITE);
        ...
        Color[] colors = {Color.BLUE, Color.BROWN, Color.CYAN, Color.FIREBRICK, Color.DARK_GRAY, Color.GREEN, Color.MAGENTA, Color.SCARLET, Color.ORANGE};
        
        for(int cols = 0; cols < totalCols; cols++){
            
            for(int rows = 0; rows < totalRows; rows++){
                
                float x = marginX + brickWidth * cols;
                float y = marginY + brickHeight * rows;
                new Brick(x, y, mainStage, colors[(int)(Math.random() * (colors.length - 1))]);
                shuffleArray(colors); 
